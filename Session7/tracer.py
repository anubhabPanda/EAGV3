...
"""
    ...
    def memory_remember(self, raw_text: str, kind: str, keywords: list[str], embedded: bool = False) -> None:
        """
        ...
        Args:
            ...
            embedded: Whether an embedding was computed for this item
        """
        # Truncate text if too long
        display_text = raw_text if len(raw_text) <= 60 else raw_text[:57] + "..."

        embed_marker = " [+embedding]" if embedded else ""
        self._log(f'[memory.remember]  classified "{display_text}" as {kind}{embed_marker}')
        self._log(f'                   keywords: {json.dumps(keywords)}')
        self._log('')

    def memory_read(self, num_hits: int, keywords: list[str] = None, method: str = "keyword", fallback: bool = False) -> None:
        """
        ...
        Args:
            ...
            method: Retrieval method used ("vector", "keyword", or "hybrid")
            fallback: Whether this was a fallback to keyword search
        """
        method_label = f" ({method})" if method != "keyword" else ""
        fallback_note = " [vector→keyword fallback]" if fallback else ""

        if num_hits == 0:
            self._log(f'[memory.read]    no relevant memories found{method_label}{fallback_note}')
        elif num_hits == 1:
            self._log(f'[memory.read]    retrieved 1 memory hit{method_label}{fallback_note}')
        else:
            self._log(f'[memory.read]    retrieved {num_hits} memory hits{method_label}{fallback_note}')
    ...
    def vector_search(self, query: str, num_results: int, index_size: int, avg_similarity: float = None) -> None:
        """
        ...
        Args:
            ...
            index_size: Size of the FAISS index
            avg_similarity: Average similarity score (optional)
        """
        query_display = query if len(query) <= 50 else query[:47] + "..."

        similarity_note = f", avg similarity: {avg_similarity:.3f}" if avg_similarity is not None else ""
        self._log(f'[vector.search]  query: "{query_display}"')
        self._log(f'                 index size: {index_size}, results: {num_results}{similarity_note}')

    def vector_embed(self, text: str, dimension: int, task_type: str = "retrieval_document") -> None:
        """
        Log an embedding computation.
        ...
        """
        text_display = text if len(text) <= 50 else text[:47] + "..."
        self._log(f'[vector.embed]   "{text_display}" -> {dimension}d vector ({task_type})')

    def vector_index_add(self, item_id: str, index_size_after: int) -> None:
        """
        Log adding an item to the FAISS index.

        Args:
            item_id: Memory item ID being indexed
            index_size_after: Index size after addition
        """
        self._log(f'[vector.index]   added {item_id} (index size now: {index_size_after})')

    def vector_index_rebuild(self, num_items: int, duration_ms: float = None) -> None:
        """
        Log rebuilding the FAISS index.

        Args:
            num_items: Number of items in the rebuilt index
            duration_ms: Time taken to rebuild (optional)
        """
        duration_note = f" ({duration_ms:.1f}ms)" if duration_ms is not None else ""
        self._log(f'[vector.index]   rebuilt from {num_items} items{duration_note}')

    def document_indexed(self, path: str, chunks: int, source: str) -> None:
        """
        Log document indexing operation.

        Args:
            path: Path to indexed document
            chunks: Number of chunks created
            source: Source identifier
        """
        self._log(f'[doc.index]      "{path}" -> {chunks} chunks indexed ({source})')

    def knowledge_search(self, query: str, num_results: int, sources: list[str] = None) -> None:
        """
        ...
        Args:
            ...
            sources: List of source identifiers in results
        """
        query_display = query if len(query) <= 50 else query[:47] + "..."
        self._log(f'[knowledge.search] query: "{query_display}" -> {num_results} chunks')
        ...
        if sources:
            ...
            self._log(f'                   sources: {sources_str}')
