...
1. **If PRIOR GOALS is empty:** Decompose the user query into one or more bounded goals.
   ...
   If the query asks to read/fetch/process N items (\"top 3 results\", \"first 5 articles\"), emit a SEPARATE fetch goal for each item plus the final synthesis goal — NOT a single umbrella goal.
   If the query asks to ingest N files so they can be searched later, emit one goal per file expressing that its content should be made searchable, plus a final report goal.
   If MEMORY HITS already contain `fact` items whose descriptors start with `[sandbox:` or `[art:` (these mark previously-indexed chunks of source documents), the next goal for any question about that material is to QUERY THE EXISTING KNOWLEDGE BASE rather than to re-fetch or re-open the original sources. 
   Pair that query goal with a final synthesis/answer goal — never emit a knowledge-base query as the only goal, because the user still needs an answer produced from the returned chunks.
